...
- [x] 2.3 Update `frontend/src/app/layout.tsx`: add `suppressHydrationWarning` to `<html>`, wrap `{children}` with `ThemeProvider`
...
- [x] 5.3 Write `frontend/e2e/theme-toggle.spec.ts`: toggling the theme updates the page's rendered state (e.g. `dark` class on `<html>`)
- [x] 5.4 Extend the e2e test to reload the page after toggling and assert the selected theme persists
